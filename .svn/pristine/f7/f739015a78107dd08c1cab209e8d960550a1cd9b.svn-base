...
use app\admin\model\ProjectType as ProjectTypeModel;
class ProjectType extends Controller
{
    ...
    public function add()
    {
        ...
        if($this->request->isAjax())
        {
            ...
            $r = $model->add($post);
            if($r){
                $this->add_SystemLog('项目类型', '增加', $post['name']);
    ...
    public function update()
    {
        ...
        if($this->request->isAjax())
        {
            ...
            $r = $model->edit($post);
            if($r !== false){
                $this->add_SystemLog('项目类型', '修改', $post['name']);
                return ['ret'=>200, 'msg'=>'操作成功'];
            }
        }else{
            $get = $this->getData();
    ...
    public function destroy()
    {
        ...
        if($this->request->isAjax())
        {
            ...
            if($post['id'] && $info){
                ...
                if($r){
                    $this->add_SystemLog('项目类型', '删除', $info['name']);
